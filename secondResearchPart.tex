\chapter{Potential energy conservation}\label{research2}
...
In table \ref{table:EnergyConAP} the energy consumption of a month of an AP has been calculated with the access point idling with WiFi on and with WiFi off using 100mb transmit sizes. 
...
Very visible is that the newer standards end up consuming more energy due to higher idling energy consumption. This difference is eliminated when the AP is turned off if not in use instead of idling.
...
\section{Switching}\label{section:SwitchingDynamically}
