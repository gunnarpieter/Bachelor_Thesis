...
\chapter{Energy and power the difference}
...
\chapter{Measurement code explained}\label{appendixB}
Within this appendix we will explain the ins and outs of the code that is used to measure the amount of energy draw by the AP using the voltage/current v2 bricklets API\footnote{\url{https://www.tinkerforge.com/en/doc/Software/Bricklets/VoltageCurrentV2_Bricklet_C.html#voltage-current-v2-bricklet-c-api}}. The code can be found on GitLab\footnote{\url{https://gitlab.science.ru.nl/gnoordbruis/EnergyConsumption}}\newline 
\newline
Within this program we use libcurl\footnote{\url{https://curl.haxx.se/}} to fetch the files from our server onto our query device. The code segments below are all code surrounding the retrieval of the file within the experiment and is mainly setting up curl and executing it.
\begin{lstlisting}
...
The idle power consumption of the access point has also been measured in section \ref{researchExperiment}. The Voltage current bricklet calls the callback function every 10ms. One second is 1000ms thus we need a 100 measurements to have the results of 1 second. This while loop prints the progress this is due to a problem mentioned at the end of section \ref{powerConsumptionMeasurement}. 
\begin{lstlisting}
void idle(){
    int second = 100;
    int minute = 6000;

	while (dataVector.size() <= (minute)) {  
		cout <<  "Size: " << dataVector.size() << " needed size: " << minute << "\n";
	}
}
\end{lstlisting}
\chapter{Raw measurement results}\label{appendixC}
\section{Experiment results 2.4Ghz}
%1MB:
\begin{table}[H]
...
\section{Experiment results 5Ghz}
\begin{table}[H]
...
\chapter{Calculation of access point energy consumption per month}\label{StandardAPpowerconsumption}
\begin{table}[H]
